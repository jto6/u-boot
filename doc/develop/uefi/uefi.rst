...
is also very important to create an EFI Signature List (ESL) File that
includes the certificate corresponding to the signed capsules. See
:ref:`Enabling Capsule Authentication`. *DO NOT* reuse the ESL file from
the reference board, which usually would use the development board keys.
